...
- Home base lives in `settings` (`home_base_address/lat/lng`). `POST /api/jobs/:id/return-log` adds the drive home as the **same job's return log** (`jobs.return_log_id`, a manual log at the home base, no photo, labelled with the home address); with an optional `distanceKm` the return reading is set to the client reading + distance. The client prefills that distance from `GET /api/distance` (OSRM driving km between the client point and home, cached in `route_cache`). A trip with a return log claims the whole round trip: `tripEndKm()` in `shared/claims.ts` picks the return reading as the trip end (a return leg without a reading keeps the trip incomplete, so it can't be claimed early). Start/end/return logs must stay in time and reading order; only one return log per trip; claimed trips must be reopened first.
- `POST /api/jobs/:id/next-trip` ("Add a trip on") creates a new **business** job starting from the current trip's last log — the return (home) log when present, else the client arrival — copying time, location point/label and odometer reading, with `event_uid` null so the original booking still maps to one trip.
- Logs carry `location_label` (the picked address) alongside their coordinates; the pickers send it and the trip popup shows it, so a manual/copied log reads as an address rather than raw GPS.
...
- `server/src/services/` — ical fetch/parse, exif GPS, photo storage (`{DATA_DIR}/photos/log-<id>/full.jpg` + `thumb.jpg`; every upload is normalized to a 1920px JPEG, HEIC/HEIF decoded via `heic-decode`), reading rules, calendar sync, job assembly, NZ geocoding (`geocode.ts`: Postgres-cached Nominatim, 1 req/s throttle; pure parse/normalize in `lib/nominatim.ts`), road distance (`routing.ts`: Postgres-cached OSRM driving distance, 1 req/s throttle; pure parse/key in `lib/osrm.ts`). `lib/tripLegs.ts` holds the pure builders behind `POST /api/jobs/:id/return-log` and `/next-trip`.
...
- Test suites: `shared/claims.test.ts`, `shared/time.test.ts`, `server/test/ical.test.ts`, `server/test/mappers.test.ts`, `server/test/validation.test.ts`, `server/test/geocode.test.ts`, `server/test/osrm.test.ts`, `server/test/tripLegs.test.ts` — extend these patterns rather than inventing new harnesses.
...
- Road distance is `GET /api/distance?fromLat&fromLng&toLat&toLng` → `{km}` (whole km, public OSRM demo, ~11 m coordinate rounding as the cache key in `route_cache`, 1 req/s throttle). Failures surface as 502 and the client just leaves the distance picker manual.
