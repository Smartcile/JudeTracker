...
- GPS sources: `live` (browser geolocation), `exif` (read from photo), `manual` (typed coordinates override via the trip popup or capture wizard — manual sets also null the stored accuracy), `none`. Clearing a log's coords switches it back to `none`.
...
- `server/src/services/` — ical fetch/parse, exif GPS, photo storage (`{DATA_DIR}/photos/log-<id>/full.jpg` + `thumb.jpg`; every upload is normalized to a 1920px JPEG, HEIC/HEIF decoded via `heic-decode`), reading rules, calendar sync, job assembly.
- `client/src/` — vanilla TS, no framework. `dom.ts` `h()` hyperscript helper. Pages: `pages/dashboard.ts` (phone logging), `pages/review.ts` (laptop: readings + claims + totals), `pages/settings.ts` (vehicles, calendar, PIN, export), `pages/calendar.ts` (month view of booked clients), `pages/faq.ts` (IRD FAQ, content in `data/faq.ts`). Components: `components/captureWizard.ts` (Take photo / existing photo / "Later on" no-photo flow, auto-saves, optional manual backdate date/time + location), `components/tripModal.ts` (the shared editable trip popup used by every list — per-slot date/time + GPS overrides, car picker/add, calendar-event linker with search and month-browse modes, photo capture, readings, claims, delete), `dial.ts` (odometer dial modal), `pinpad.ts`, `modal.ts`, `toast.ts`, `useSplit.ts` (per-vehicle use analysis rows).
- Test suites: `shared/claims.test.ts`, `server/test/ical.test.ts`, `server/test/mappers.test.ts`, `server/test/validation.test.ts` — extend these patterns rather than inventing new harnesses.
